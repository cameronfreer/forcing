...
- **"Adds a new real" is used exactly at the certified material theorem**
  ...
  **avoidance**: the abstract interface
  earns its over-`M` phrase exactly through its visibility obligations (`Sees` —
  every designated real's diagonal test visible), and its formal conclusion is
  `c_G ∉ M.designatedReals` — a designated family, not the reals of a ground;
  the interface does not provide a model carrier, so it must not claim
  `c_G ∉ M`. The purely external statement is weaker still: "the generic
  diagonalizes a supplied countable family." The material Cohen adapter
  discharges the pieces separately, and each is now certified: the designated
  reals are derived as the materially coded reals (`derivedContext`), the test
  visibility is proved from carried internal codes (`sees_derivedContext`),
  and external countability enters only where generic existence is proved
  (`addsNewReal`).
