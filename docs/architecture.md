...
7. **External complete Boolean algebras are distinct from algebras complete only
   ...
   theorems are restricted to `M`-names accordingly. Reserve the unqualified
   phrase "Boolean completion" for the external notion.
...
- **"Adds a new real" is reserved for the over-`M` theorem** (M3): if `G` is
  `M`-generic then `c_G ∉ M`. The purely external statement is only "the generic
  diagonalizes a supplied countable family." The abstract M3 interface earns the
  over-`M` phrase exactly through its closure hypothesis (every ground real's
  diagonal dense set belongs to `M`'s dense family); the CTM instantiation later
  discharges that hypothesis via absoluteness.
