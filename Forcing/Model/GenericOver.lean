...
The last theorem records where the ground model's closure properties first matter: meeting
every visible *dense* set is a priori a stronger demand than `GenericOver`, because the
persistent form of a visible test need not itself be visible. The two agree when the visible
family is closed under downward closure — stated as an explicit hypothesis, deliberately not a
...
Without the closure hypothesis only the forward direction of the underlying trade survives —
the persistent form of a visible test need not be visible. The closure is an explicit
hypothesis, deliberately not a field of `GroundContext`. -/
theorem genericOver_iff_forall_isDense
    (hM : ∀ D, M.Visible D → M.Visible ↑(lowerClosure D)) :
    GenericOver M G ↔ ∀ D, M.Visible D → IsDense D → Meets G D := by
  rw [genericOver_iff]
  refine ⟨fun h D hD hdense ↦ ?_, fun h D hD hDO ↦ h D hD hDO.1⟩
  exact meets_lowerClosure.1 (h _ (hM D hD) hdense.isDenseOpen_lowerClosure)
