...
Factored out when its second consumer arrived (formula coding, after the Cohen condition
...
@[simp] theorem natCode_zero : natCode 0 = ∅ :=
  rfl

/-- The successor code is an `insert` — the shape material-membership arguments recurse on. -/
@[simp] theorem natCode_succ (n : ℕ) : natCode (n + 1) = insert (natCode n) (natCode n) :=
  rfl
