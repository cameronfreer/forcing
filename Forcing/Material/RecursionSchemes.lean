...
This module owns the **aggregation layer**: stages, rows, and graphs relative to a *history*.
The fixed-point construction, and the eleven further instances it needs, live in
`Forcing/Material/RecursionExistence.lean`.

The instances owned here, each named by its job:
...
The count is **discovery-driven** throughout: it is read off the construction that compiled,
not chosen in advance. Six here, eleven in the existence module, **seventeen** in total.
...
* `Forcing.AtomicRecursion.entryBoundFormula`, `Forcing.AtomicRecursion.stageFormula`,
  ...
  the aggregation-layer instance formulas.
...
* `Forcing.MaterialGround.exists_graphValue_coherent_of_agree`: aggregation plus the bridge,
  with the fixed-point obligation stated rather than discharged — it is discharged in
  `Forcing/Material/RecursionExistence.lean`.
-/
